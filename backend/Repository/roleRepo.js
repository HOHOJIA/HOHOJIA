...
module.exports = {
    ...
    addRoleToUser: async (roleId , userId,connection) => {
        try {
            const addRoleToUserQuery = 'INSERT INTO userRole(roleId, userId) VALUES(?,?)';
    ...
    checkRole: async (userId)=>{
        ...
        try {
            const selectQuery = 'SELECT * FROM userRole AS u LEFT JOIN roles AS r ON u.roleId = r.id WHERE u.userId = ?'
