...
async function getImgUrl(file: File) {
    ...
    try {
        const res = await fetch(`http://13.210.223.164/api/generate-presigned-url?${queryParams}`)
...
export default function Post() {
    ...
    const [ingredients, setIngredients] = useState([{ id: uuidv4(), name: '', size: '' }])
    // 創建一個狀態來保存選中的標籤
    ...
    function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        formData.append(
            'imgUrl',
            totalImgUrl.length > 0 ? totalImgUrl[0] : 'https://images.dog.ceo/breeds/appenzeller/n02107908_3450.jpg'
        )
    ...
    return (
        <>
            ...
            <div className="flex justify-center w-full h-full bg-slate-50">
                <Card shadow="md" className="w-full py-4 my-4 md:w-10/12 lg:w-8/12 lg:my-12">
                    <form onSubmit={handleSubmit} onKeyDown={handleKeyDown}>
                        ...
                        <CardBody className="flex-col items-start px-8 py-4 sm:px-12">
                            <p className="mb-4 text-xl font-bold border-b-2 border-black">料理細節</p>
                            {/* Description section */}
                            <Description ingredients={ingredients} setIngredients={setIngredients} />
                            ...
                            <div className="w-full">
                                ...
                                <Textarea minRows={5} label="輸入食譜描述（最多150字）" variant="bordered" name="tip" />
                            </div>
